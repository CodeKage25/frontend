...
import { getProduct, requestCallBack } from "@/api/product/endpoints";
...
const RightPane = () => {
    ...
    const handleRequestCallBack = async () => {
        try {
            const providerId = 2;
            await requestCallBack(providerId);
            toast.success("Callback requested successfully!", {
              position: toast.POSITION.TOP_CENTER
            });
          } catch (error) {
          toast.error("Error requesting callback. Please try again.", {
            position: toast.POSITION.TOP_CENTER
          });
    ...
    return (
        <section className="flex flex-col">
            <div className="flex flex-col rounded-xl px-7 py-4 border border-[#828282] mt-9">
                ...
                <Button 
                className="font-semibold text-base text-primary p-0" 
                variant="link"
